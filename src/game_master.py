...
from entity import Room, Player, State
...
class GameMaster:
    ...
    async def process_command(self, command: dict, player: Player):
        ...
        if room.latest_state == State.ROUND_LOCKED:
             await self.notify_user({"message":"Round locked. You can't choose"}, player.token)
             return
        ...
        if len(room.rounds[room.round_number]) == 2:
            ...
            delay = 1
            await self.notify_room(f"Round {room.round_number} will be closed in next {delay} seconds. "
    ...
    def can_game_be_over(self, room_id) -> bool:
        ...
        if len(room.rounds) >= need_to_win:
            counter = Counter(game_round['winner'] for game_round in room.rounds.values() if 'winner' in game_round).items()
            if len(counter) != 2:
                return False
    ...
    async def lock_round_with_delay(self, room_id: int, delay=3):
        ...
        for sec in range(delay, 0, -1):
            await self.notify_room(f"Round {room.round_number} will be finished in {sec}...", room_id)
